...
(defun tcp-accept (server &key (timeout 30))
  "Waits up to TIMEOUT (defaults to 30) seconds for a client to connect to SERVER.
   A new socket is returned, which is bound to the client connection.
   If a timeout occured, nil is returned."
  (when server
...
(defun tcp-connect (server port &optional (timeout 5))
  "Returns a socket connected to SERVER:PORT.
   If an error occurs, or the attempt times out after TIMOUT (default 5) secons, nil is returned."
...
(defun tcp-print-raw (socket line)
  ...
  (when (and socket line)
    (sb-bsd-sockets:socket-send socket (concatenate 'string line (list #\return  
							#\newline)) nil)))

(defun tcp-read-raw (socket &key (maxsize 65536) (timeout 5))
  "Reads one line from SOCKET, removes CRLF, and returns it.
   The buffer size is 65536 bytes, unless MAXSIZE is specified.
   If no result is received within TIMEOUT seconds (defaults to 5), nil is returned."
  (when socket
